  ...
  <div class="min-h-[calc(100vh-120px)] py-10 px-4" style="background:#FAFAF7;">
    <div class="max-w-6xl mx-auto">
      ...
      <div class="mb-8 flex items-center gap-3">
        ...
        <div>
          <h1 class="text-2xl font-bold" style="color:#1A1A1A;">Shopping Cart</h1>
          <p class="text-sm" style="color:#A8A8A8;">
      ...
      <div v-if="cartStore.items.length === 0" class="flex flex-col items-center justify-center py-24 text-center">
        <div class="w-24 h-24 rounded-3xl flex items-center justify-center mb-6"
          ...
          <svg class="w-12 h-12" style="color:#A8A8A8;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
              d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/>
          </svg>
        </div>
        <h2 class="text-xl font-bold mb-2" style="color:#1A1A1A;">Your cart is empty</h2>
        <p class="text-sm mb-8 max-w-xs" style="color:#6B6B6B;">
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 flex flex-col gap-3">
          ...
          <TransitionGroup name="cart-item" tag="div" class="flex flex-col gap-3">
            <div v-for="item in cartStore.items" :key="item.key"
              ...
              <div class="flex-1 min-w-0">
                ...
                <p v-if="item.variationLabel" class="text-[12px] mt-0.5" style="color:#A8A8A8;">
                  {{ item.variationLabel }}
                </p>
                <p class="text-base font-bold mt-1" style="color:#1A1A1A;">
                  ${{ (item.price * item.qty).toFixed(2) }}
                </p>
                <p class="text-[11px]" style="color:#A8A8A8;">
              ...
              <div class="flex flex-col items-end gap-3 flex-shrink-0">
                <button @click="removeFromCart(item.key)"
                  ...
                  style="color:#A8A8A8;"
                  @mouseenter="(e) => { e.currentTarget.style.background='#F4ECE6'; e.currentTarget.style.color='#A4351A'; }"
                  @mouseleave="(e) => { e.currentTarget.style.background=''; e.currentTarget.style.color='#A8A8A8'; }">
                  <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                  </svg>
                </button>

                <div class="flex items-center rounded-xl overflow-hidden" style="border:1.5px solid #E4E0D8;">
                  ...
                  <span class="w-9 text-center text-sm font-bold" style="color:#1A1A1A;border-left:1px solid #E4E0D8;border-right:1px solid #E4E0D8;">
                    {{ item.qty }}
                  </span>
                  <button @click="updateQty(item.key, item.qty + 1)"
                    class="w-8 h-8 flex items-center justify-center transition-colors duration-150 text-gray-500 hover:bg-gray-50">
          ...
          <div class="flex justify-end mt-1">
            <button @click="confirmClear = true"
              ...
              style="color:#A8A8A8;"
              @mouseenter="(e) => e.currentTarget.style.color='#A4351A'"
              @mouseleave="(e) => e.currentTarget.style.color='#A8A8A8'">
              Clear cart
            </button>
          </div>
        </div>
        ...
        <div class="lg:col-span-1">
          <div class="bg-white rounded-2xl p-6 sticky top-24" style="border:1px solid rgba(0,0,0,0.06);">
            <h2 class="text-base font-bold mb-5" style="color:#1A1A1A;">Order Summary</h2>

            <div class="space-y-3 text-sm">
              <div class="flex justify-between">
                <span style="color:#6B6B6B;">Subtotal ({{ cartCount }} items)</span>
              ...
              <div v-if="shipping > 0" class="text-[12px] px-3 py-2 rounded-xl" style="background:#eff6ff;color:#16358F;">
            ...
            <div class="mt-5">
              <div class="flex gap-2">
                ...
                <button @click="applyCoupon"
                  ...
                  style="background:#F0EDE6;color:#3F3F3F;"
                  @mouseenter="(e) => { e.currentTarget.style.background='#1D3FB8'; e.currentTarget.style.color='#fff'; }"
                  @mouseleave="(e) => { e.currentTarget.style.background='#F0EDE6'; e.currentTarget.style.color='#3F3F3F'; }">
                  Apply
                </button>
              </div>
              <p v-if="couponMsg" class="text-[12px] mt-1.5 font-medium" :style="{ color: couponError ? '#A4351A' : '#22c55e' }">
                {{ couponMsg }}
              </p>
            </div>
            ...
            <!-- Trust row -->
            <div class="flex items-center justify-center gap-5 mt-5 pt-4" style="border-top:1px solid #F0EDE6;">
              <div v-for="t in trust" :key="t.label" class="flex items-center gap-1.5">
                <svg class="w-3.5 h-3.5" style="color:#A8A8A8;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" :d="t.path"/>
                </svg>
                <span class="text-[11px] font-medium" style="color:#A8A8A8;">{{ t.label }}</span>
              </div>
            </div>
          </div>
        </div>
    ...
    <Transition name="fade">
      <div v-if="confirmClear"
        ...
        <div class="bg-white rounded-2xl p-6 w-full max-w-xs shadow-2xl">
          <h3 class="text-base font-bold mb-1" style="color:#1A1A1A;">Clear cart?</h3>
          <p class="text-sm mb-5" style="color:#6B6B6B;">This will remove all items from your cart.</p>
          <div class="flex gap-3">
            <button @click="confirmClear=false"
              class="flex-1 py-2.5 rounded-xl text-sm font-bold transition-colors"
              style="border:1.5px solid #E4E0D8;color:#3F3F3F;">
              Cancel
            </button>
            <button @click="doClear"
              class="flex-1 py-2.5 rounded-xl text-sm font-bold text-white transition-colors"
              style="background:#A4351A;">
              Clear All
            </button>
          </div>
        </div>
