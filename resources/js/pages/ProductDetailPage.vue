<template>
  ...
  <div v-else-if="!product" class="min-h-screen flex items-center justify-center text-center px-4">
    <div>
      <h2 class="text-2xl font-bold mb-2" style="color:#1A1A1A;">Product not found</h2>
  ...
  <main v-else>
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 xl:gap-20">
            ...
            <div v-if="productGallery.length > 1" class="flex gap-3 mt-4 overflow-x-auto pb-1">
              <button v-for="(url, i) in productGallery" :key="i"
                ...
                :style="effectiveThumb === url
                  ? 'border:2px solid #1D3FB8; box-shadow:0 0 0 2px rgba(29,63,184,0.2);'
          ...
          <div class="flex flex-col gap-6">
            ...
            <!-- Name -->
            <h1 class="text-3xl sm:text-4xl font-bold leading-tight" style="color:#1A1A1A;">
              {{ product.name }}
            </h1>
            ...
            <div v-if="product.rating > 0" class="flex items-center gap-3">
              ...
              <span class="text-sm font-semibold" style="color:#3F3F3F;">{{ product.rating }}</span>
            ...
            <div v-if="product.type === 'simple'" class="flex items-baseline gap-3">
              <span class="text-3xl font-bold" style="color:#1A1A1A;">${{ product.price }}</span>
              <span v-if="product.original_price" class="text-lg text-gray-400 line-through">${{ product.original_price }}</span>
              <span v-if="discount" class="text-xs font-semibold px-2 py-1 rounded-md" style="background:#F3EAD2;color:#8B5A1F;">Save {{ discount }}%</span>
            </div>
            <!-- Price: variable -->
            <div v-else-if="product.type === 'variable'" class="flex items-baseline gap-3">
              <template v-if="selectedVariation">
                <span class="text-3xl font-bold" style="color:#1A1A1A;">${{ selectedVariation.price }}</span>
              ...
              <template v-else>
                <span class="text-3xl font-bold" style="color:#1A1A1A;">{{ variablePriceRange ?? '—' }}</span>
            ...
            <div v-else-if="groupPriceLabel" class="flex items-baseline gap-3">
              <span class="text-3xl font-bold" style="color:#1A1A1A;">{{ groupPriceLabel }}</span>
            ...
            <div class="grid grid-cols-3 gap-3 mt-2">
              <div v-for="t in trust" :key="t.label" class="trust-item">
                <svg class="w-5 h-5 mb-1.5" style="color:#6B6B6B;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" :d="t.path"/>
                </svg>
                <span class="text-[11px] font-semibold text-center leading-tight" style="color:#3F3F3F;">{{ t.label }}</span>
              </div>
            </div>
    ...
    <!-- Related products -->
    <section v-if="related.length > 0" class="py-16" style="background:#FAFAF7;">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-10">
          <h2 class="text-2xl font-bold" style="color:#1A1A1A;">You may also like</h2>
        </div>
...
.btn-primary:hover { background: #16358F; }
...
.discount-chip {
  ...
  background: #B5532C;
  box-shadow: 0 4px 12px rgba(181,83,44,0.4);
...
.qty-btn:hover { background: #F0EDE6; color: #1A1A1A; }
.qty-num {
...
.cart-btn:hover { background: #16358F; }
.cart-btn-done {
...
.wish-btn:hover { border-color: #D9A595; color: #A4351A; }
.wish-active { color: #A4351A; border-color: #D9A595; background: #F4ECE6; }
