  ...
  <section class="cat-hero">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <nav class="flex items-center gap-2 text-sm mb-4" style="color:#A8A8A8;">
        ...
        <span>/</span>
        <span style="color:#1A1A1A;">{{ category?.name ?? 'Category' }}</span>
      </nav>
      ...
      <template v-else-if="category">
        <h1 class="text-3xl sm:text-4xl font-bold" style="color:#1A1A1A;">{{ category.name }}</h1>
      ...
      <!-- Not found -->
      <div v-else class="text-xl font-bold" style="color:#1A1A1A;">Category not found</div>
    </div>
  </section>
