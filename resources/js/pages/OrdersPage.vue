  ...
  <div class="min-h-[calc(100vh-120px)] px-4 py-10" style="background:#FAFAF7;">
    <div class="max-w-3xl mx-auto">
      ...
      <div class="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 class="text-2xl font-bold" style="color:#1A1A1A;">My Orders</h1>
          <p class="text-sm" style="color:#A8A8A8;">Track and manage your orders</p>
        </div>
      </div>

      <!-- Not logged in -->
      <div v-if="!authStore.user" class="text-center py-24">
        <div class="inline-flex w-20 h-20 rounded-2xl items-center justify-center mb-6" style="background:#F0EDE6;">
          <svg class="w-10 h-10" style="color:#A8A8A8;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
              d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
          </svg>
        </div>
        <h2 class="text-xl font-bold mb-2" style="color:#1A1A1A;">Sign in to view your orders</h2>
        <p class="text-sm mb-6" style="color:#6B6B6B;">You need to be logged in to see your order history.</p>
        <RouterLink to="/login"
      ...
      <div v-else-if="orders.length === 0" class="text-center py-24">
        ...
        <h2 class="text-xl font-bold mb-2" style="color:#1A1A1A;">No orders yet</h2>
        <p class="text-sm mb-6" style="color:#6B6B6B;">When you place an order it will appear here.</p>
        <RouterLink to="/shop"
          class="px-6 py-2.5 rounded-xl text-sm font-semibold text-white"
          style="background:#1D3FB8;">
          Start Shopping
        ...
        <div v-for="order in orders" :key="order.id"
          ...
          <div class="flex items-start justify-between gap-3 mb-4">
            <div>
              ...
              <p class="text-xs mt-0.5" style="color:#A8A8A8;">{{ fmtDate(order.created_at) }}</p>
            </div>
          ...
          <div class="flex flex-wrap gap-2 mb-4">
            <div v-for="item in order.items" :key="item.id"
              ...
              <span style="color:#A8A8A8;">× {{ item.quantity }}</span>
            </div>
          </div>

          <!-- Footer row -->
          <div class="flex items-center justify-between pt-3" style="border-top:1px solid #F0EDE6;">
            <div>
              <span class="text-xs" style="color:#A8A8A8;">Total</span>
            ...
            <RouterLink :to="`/orders/${order.id}`"
              ...
              @mouseenter="(e) => { e.currentTarget.style.borderColor='#1D3FB8'; e.currentTarget.style.color='#1D3FB8'; }"
              @mouseleave="(e) => { e.currentTarget.style.borderColor='#E4E0D8'; e.currentTarget.style.color='#3F3F3F'; }">
              View Details
