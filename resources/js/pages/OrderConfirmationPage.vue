  ...
  <div class="min-h-[calc(100vh-120px)] flex items-center justify-center px-4 py-16"
    ...
    <div class="w-full max-w-lg">
      ...
      <div v-if="loading" class="text-center py-16">
        ...
        <p class="text-sm" style="color:#A8A8A8;">Loading your order…</p>
      </div>
      ...
      <template v-else-if="order">
        ...
        <div class="text-center mb-8">
          ...
          <h1 class="text-2xl font-bold mb-2" style="color:#1A1A1A;">Thanks, your order is confirmed</h1>
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm mb-4" style="border:1px solid rgba(0,0,0,0.06);">
          ...
          <!-- Order meta -->
          <div class="grid grid-cols-2 gap-4 mb-5 pb-5" style="border-bottom:1px solid #F0EDE6;">
            <div>
              <p class="text-[11px] font-semibold uppercase tracking-wider mb-1" style="color:#A8A8A8;">Order Number</p>
              <p class="text-base font-bold" style="color:#1A1A1A;">#{{ String(order.id).padStart(5, '0') }}</p>
            </div>
            <div>
              <p class="text-[11px] font-semibold uppercase tracking-wider mb-1" style="color:#A8A8A8;">Status</p>
            ...
            <div>
              <p class="text-[11px] font-semibold uppercase tracking-wider mb-1" style="color:#A8A8A8;">Order Date</p>
              <p class="text-sm font-semibold" style="color:#1A1A1A;">{{ orderDate }}</p>
            </div>
            <div>
              <p class="text-[11px] font-semibold uppercase tracking-wider mb-1" style="color:#A8A8A8;">Total</p>
              <p class="text-base font-bold" style="color:#1A1A1A;">${{ parseFloat(order.total).toFixed(2) }}</p>
            </div>
          </div>

          <!-- Items -->
          <h3 class="text-sm font-bold mb-3" style="color:#1A1A1A;">Items Ordered</h3>
          <div class="space-y-2.5 mb-5">
            <div v-for="item in order.items" :key="item.id"
              class="flex items-center justify-between gap-3">
              <div class="flex items-center gap-3 min-w-0">
                <div class="w-9 h-9 rounded-xl overflow-hidden flex-shrink-0" style="background:#F0EDE6;">
                ...
                <div class="min-w-0">
                  <p class="text-sm font-medium truncate" style="color:#1A1A1A;">{{ item.product_name }}</p>
                  <p class="text-[11px]" style="color:#A8A8A8;">Qty: {{ item.quantity }}</p>
                </div>
              </div>
          ...
          <!-- Shipping address -->
          <div class="pt-4" style="border-top:1px solid #F0EDE6;">
            <p class="text-[11px] font-bold uppercase tracking-wider mb-2" style="color:#A8A8A8;">Shipping To</p>
        ...
        <div class="flex items-center gap-3 px-4 py-3.5 rounded-xl mb-6"
          style="background:rgba(29,63,184,0.06);border:1px solid rgba(29,63,184,0.12);">
          <svg class="w-4 h-4 flex-shrink-0" style="color:#1D3FB8;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
          </svg>
          <p class="text-sm" style="color:#16358F;">
      ...
      <div v-else class="text-center py-16">
        ...
        <RouterLink to="/shop" class="text-sm font-semibold" style="color:#1D3FB8;">Back to Shop</RouterLink>
      </div>
