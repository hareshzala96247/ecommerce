    ...
    <div class="max-w-2xl mx-auto">
      ...
      <div v-else-if="!authStore.user" class="text-center py-24">
        <div class="inline-flex w-16 h-16 rounded-2xl items-center justify-center mb-6"
          ...
          <svg class="w-8 h-8" style="color:#A8A8A8;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
          </svg>
        </div>
        <h2 class="text-xl font-bold mb-2" style="color:#1A1A1A;">Sign in to your account</h2>
        <p class="text-sm mb-6" style="color:#6B6B6B;">Access your orders, wishlist, and profile settings.</p>
        <div class="flex gap-3 justify-center">
          <RouterLink to="/login"
            class="px-6 py-2.5 rounded-xl text-sm font-bold text-white transition-all duration-200"
            style="background:#1D3FB8; box-shadow:0 4px 12px rgba(29,63,184,0.3);">
            Sign In
          </RouterLink>
          <RouterLink to="/register"
            class="px-6 py-2.5 rounded-xl text-sm font-bold transition-all duration-200"
            style="border:1.5px solid #e5e7eb; color:#3F3F3F;">
            Create Account
          </RouterLink>
        </div>
      </div>
      ...
      <template v-else>
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm mb-4" style="border:1px solid rgba(0,0,0,0.06);">
          <div class="flex items-center gap-4">
            ...
            <div class="min-w-0">
              <h1 class="text-xl font-bold truncate" style="color:#1A1A1A;">{{ authStore.user.name }}</h1>
              <p class="text-sm truncate" style="color:#6B6B6B;">{{ authStore.user.email }}</p>
              <span class="inline-flex items-center mt-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold"
                style="background:rgba(29,63,184,0.1);color:#1D3FB8;">
                Customer
              </span>
            </div>
            <button @click="handleLogout" :disabled="loggingOut"
              ...
              style="border:1.5px solid #F4ECE6;color:#A4351A;"
              @mouseenter="(e) => e.currentTarget.style.background='#F4ECE6'"
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
          <RouterLink v-for="item in quickLinks" :key="item.label" :to="item.to"
            ...
            <div>
              <p class="text-sm font-bold" style="color:#1A1A1A;">{{ item.label }}</p>
              <p class="text-[12px]" style="color:#A8A8A8;">{{ item.desc }}</p>
            </div>
          </RouterLink>
        </div>
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm mt-4" style="border:1px solid rgba(0,0,0,0.06);">
          <h2 class="text-base font-bold mb-4" style="color:#1A1A1A;">Account Details</h2>
          <div class="space-y-3">
            <div class="flex items-center justify-between py-2.5"
              style="border-bottom:1px solid #F0EDE6;">
              <span class="text-sm" style="color:#6B6B6B;">Full Name</span>
              <span class="text-sm font-semibold" style="color:#1A1A1A;">{{ authStore.user.name }}</span>
            </div>
            <div class="flex items-center justify-between py-2.5"
              style="border-bottom:1px solid #F0EDE6;">
              <span class="text-sm" style="color:#6B6B6B;">Email</span>
              <span class="text-sm font-semibold" style="color:#1A1A1A;">{{ authStore.user.email }}</span>
            </div>
            <div class="flex items-center justify-between py-2.5">
              <span class="text-sm" style="color:#6B6B6B;">Member Since</span>
              <span class="text-sm font-semibold" style="color:#1A1A1A;">{{ joinDate }}</span>
            </div>
          </div>
        </div>
      </template>
...
const quickLinks = [
    { label: 'My Orders', desc: 'Track your orders', icon: ShoppingBagIcon, bg: 'rgba(29,63,184,0.08)', color: '#1D3FB8', to: '/orders' },
    { label: 'Wishlist',  desc: 'Saved items',        icon: HeartIcon,       bg: 'rgba(164,53,26,0.08)',  color: '#A4351A', to: '/shop'   },
    { label: 'Deals',     desc: 'Exclusive offers',   icon: TagIcon,         bg: 'rgba(47,125,90,0.08)',  color: '#22c55e', to: '/shop'   },
];
</script>
